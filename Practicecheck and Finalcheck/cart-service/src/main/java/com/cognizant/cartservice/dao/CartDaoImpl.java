...
import com.cognizant.FeignClient.MenuItemFeignClient;
...
public class CartDaoImpl implements CartDao {
	...
	@Autowired
	MenuItemFeignClient feignService;
	...
	public void addCartItem(int userId, int menuItemId) throws MenuItemNotFoundException {
		...
		MenuItem item = feignService.findMenuItemById(menuItemId);
		if (item == null) {
	...
	public ArrayList<MenuItem> getAllCartItems(int userId) {
		...
		if (cartList != null) {
			for (Cart cart : cartList) {
				MenuItem menuItem = feignService.findMenuItemById(cart.getItem_id());
	...
	public void deleteCartItem(int userId, int menuItemId) {
		log.debug("START");
		MenuItem menuItem = feignService.findMenuItemById(menuItemId);
		ArrayList<Cart> cartList = cartRepository.findByUserId(userId);
		Cart delCart = null;
		for (Cart cart : cartList) {
