...
public interface CartRepository extends JpaRepository<Cart, Integer>{
	...
	@Query(nativeQuery = true, value="SELECT * from usercart uc WHERE uc.user_id = :user_id")
	public ArrayList<Cart> findByUserId(@Param("user_id") int user_id); 

	//Fetch count of items in cart
	@Query(nativeQuery = true, value="SELECT count(*) from usercart uc WHERE uc.user_id = :user_id")
