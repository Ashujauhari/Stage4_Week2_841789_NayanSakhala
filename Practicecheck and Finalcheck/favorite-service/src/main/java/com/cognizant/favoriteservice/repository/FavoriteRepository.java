...
public interface FavoriteRepository extends JpaRepository<Favorite, Integer> {
	...
	@Query(nativeQuery = true, value = "SELECT * from favorites uf WHERE uf.u_id = :user_id")
	public ArrayList<Favorite> findByUserId(@Param("user_id") int user_id);

	// Fetch count of items in cart
	@Query(nativeQuery = true, value = "SELECT count(*) from favorites uf WHERE uf.u_id = :user_id")
